...
export default ()=>{
    ...
    slide.add({
        ...
        $('._main .section03 .layout').each((_,e)=>{
            ...
            const trigger = e;
            const slideLength = $(e).find('.swiper-slide').length
                ...
                if(index === 0){
                    ...
                    tl.fromTo(e,{
                        ...
                        /* onStart : ()=>{
                            ...
                            $(trigger).find(".box ul li").eq(0).addClass('act');
                        },
                        onUpdate : trackDirection({
                            onUpdate : (direction)=>{
                                if(direction < 0){
                                    ...
                                    $(trigger).find(".box ul li").removeClass('act');
                                    $(trigger).find(".box ul li").eq(realIndex).addClass('act');
                                }
                            }
                        }) */
                    })
                        ...
                        onUpdate : trackDirection({
                            onUpdate : (direction)=>{
                                if(direction < 0){
                                    ...
                                    let realIndex = index;
                                    if(realIndex >= slideLength){
                                        realIndex = slideLength;
                                    }
                                    $(trigger).find(".box ul li").removeClass('act');
                                    $(trigger).find(".box ul li").eq(realIndex).addClass('act');
                                }
